  ...
  MPI_Status status;
    ...
    if(k>=lb && k <= ub){ /*I already have the row*/
      ...
      for(proc = 0;proc < p;proc++){ /*send the row to all other nodes */
        if(proc != id){
          MPI_Send(&rowK[0], n, MPI_INT, proc, 0, MPI_COMM_WORLD);
       //   printf("I'm %d and I've just sent row %d\n", id, k);
        }
      }
    }else{ /*I don't yet have this row */
      //printf("I'm %d, waiting for row %d\n", id, k);
      MPI_Recv(&rowK[0], n, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
