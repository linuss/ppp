...
void writeToFile(graph* g) {
	...
	if ((WRITE_TO_FILE) && (STORE_IN_MEMORY == 1)) {
		...
		fprintf(newfp, "%ld %ld %d \n", g->n, g->m, oriented);
		for (i=0; i<g->m; i++) {
			fprintf(outfp, "a %ld %ld %ld\n", g->start[i]+1, \
					g->end[i]+1, g->w[i]);
			fprintf(newfp, "%ld %ld %ld \n", g->start[i]+1, \
                                        g->end[i]+1, g->w[i]);
		}
...
void updateEdgeVal(char* FILENAME, LONG_T numEdges, LONG_T numVertices) {
...
void updateEdges(char* FILENAME, LONG_T numEdges,LONG_T numVertices, int oriented) {
        FILE* fp;
        char line[100];
	...
	fprintf(fp, "%ld %ld %d", numVertices, numEdges, oriented);
